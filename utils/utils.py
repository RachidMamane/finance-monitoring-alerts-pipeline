...
def create_annonces_index(index_name:str):
    es = get_es(4,1)
    mappings = {
        "mappings": {
            "properties" : {
                ...
                "timestamp":  {"type" : "date" , "format" : "yyyy-MM-dd HH:mm:ss ||yyyy-MM-dd HH:mm:ss.SSSSSS||yyyy-MM-dd'T'HH:mm:ss.SSS||epoch_millis"}
    ...
    logging.info(f"index {index_name} existe déja")

def index_annoucement(index_name , data):
    es = get_es(4,3)
    try : 
        doc_id = f"{data['stock']}-{data['timestamp']}"
        es.index(index = index_name , id=doc_id , document=data)
        logging.debug(f"Données boursières indexées dans ES: {data.get('stock')} - {data.get('date')}")
    except Exception as e :
        logging.error(f"Échec de l'indexation des données boursières dans Elasticsearch: {e}")
